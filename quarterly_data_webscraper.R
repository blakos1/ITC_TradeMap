#packages --------------------
{
  ...
  if(!dir.exists("quarterly_data")){
    dir.create("quarterly_data")
  }
  
  download_path = paste0(getwd(), "/quarterly_data") %>% 
    normalizePath()
...
# remDr$navigate(link2)
{
  ...
  fill_homepage = function(){
    ...
    placeholder = "ctl00_PageContent_Button_TimeSeries_Q"
    c_path = remDr$findElement(using = "id", placeholder)
    c_path$clickElement()
  ...
  for (i3 in country_list){
    ...
    timeperiod_path = '//*[@id="ctl00_PageContent_GridViewPanelControl_DropDownList_NumTimePeriod"]/option[@value = "20"]'
